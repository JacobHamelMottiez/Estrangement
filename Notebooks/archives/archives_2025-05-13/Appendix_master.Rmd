---
title: "Technical Appendix Master's Thesis"
...
This file is the technical appendix for my master thesis under the supervision of Pierre-Olivier Méthot (UL) and François Claveau (UdS). Note that the information about how we got our data is in another file :  `C:/Users/jacob/OneDrive - Université Laval/biophilo/fetch_data_2024-11-18.Rmd`. 
...
## Openalex
It is important to note that even if we did not considered it, the open database openalex, is increasingly popular. Here is some descriptive informations to document how it compares to classical databases. 
...
ggplot(full_data, aes(x = year, y = ratio))+ 
  geom_smooth() + 
  geom_line() + 
  labs(title = "Ratio of missing abstract over all documents")
```
...
articles_and_references_bioRxiv <- articles_and_references_bioRxiv |> filter(citing_year >2013 & citing_year < 2025)
...
p2 <- ggplot(delay_bio_refs |> filter(!is.na(decade)), aes(x = delay, color = decade, group = decade, fill = decade)) +
  ...
  labs(title = "Citation Delay Philosophy of Biology (only references to biology)",
    ...
    axis.title.y = element_text(size = 10),
    axis.text.y = element_text(size = 10),
    axis.text.x = element_text(size = 10)     # Increasing the size of the legend text
  ) +
  scale_x_continuous(limits = c(0, 50)) 
...
```{r , message = FALSE, warning = FALSE}
ggplot(delay_bio_refs |> filter(!is.na(decade)), aes(x = delay, color = decade, group = decade)) +
  geom_density() +
...
# Step 4: Plot
ggplot(philo_to_bio |> filter(citing_year> 1986 & citing_year < 2025) |> select(citing_year, ratio, citing_journal) |> distinct(), aes(x = citing_year, y = ratio))+ #color = citing_journal)) +
...
```{r, message = FALSE, warning = FALSE}
# Step 4: Plot
ggplot(philo_to_bio |> filter(citing_year> 1986 & citing_year < 2025), aes(x = citing_year, y = ratio, color = citing_journal)) +
  geom_line(linewidth = 2, color = "darkgrey") +
  geom_smooth(linewidth = 3) +
  labs(
...
```{r, message = FALSE, warning = FALSE}
ggplot(bio_to_philo |> filter(citing_year > 2015 & citing_year < 2025) |> select(ratio, citing_year) |> distinct(), aes(x = citing_year, y = ratio)) +
...
This part of topic-modeling has been done in a distinct Jupyter Notebook to which you can 
have access here. For the sake of simplicity, I added here the main figures relevant for analysis. 
...
- We used the "all-MiniLM-L6-v2" embedding model to transform our abstract into vectors. 
- We used UMAP to reduce those high dimension vectors into 2D for vizualisation (hyperparameters : n_neighbors=5, n_components= 2, min_dist=0.0, metric='cosine', random_state=30). 
- We used HDBSCAN for clustering (hyperparameters : min_cluster_size=10, metric='euclidean', cluster_selection_method='eom', prediction_data=True).
...
![](C:/Users/jacob/OneDrive - Université Laval/biophilo/Visualisation/BERTopic_biology_and_philosophy.png)
