---
title: "A concerning Estrangement? The case of *Biology and Philosophy* "
date: 2025-04-28
author:
...
dt <- gt(art_all_BT |>
  group_by(FROM) |>
  summarise(total_N = sum(N)) |>
  arrange(desc(total_N))) |> interactivity()
dt |> united_theme_gt()
dt |> slate_theme_gt()
...
articles_and_references_philo |> summary()
```
...
delay_bio_refs <- delay_bio_refs |> mutate(citing_journal = case_when(
    citing_journal == "STUDIES IN HISTORY AND PHILOSOPHY OF SCIENCE PART C :STUDIES IN HISTORY AND PHILOSOPHY OF BIOLOGICAL AND BIOMEDICAL SCIENCES" ~ 
      "STUDIES IN HPS",
     citing_journal == "HISTORY AND PHILOSOPHY OF THE LIFE SCIENCES" ~ "HPLS",
    TRUE ~ citing_journal
  ))
...
journals_bio_not_top_general <- articles_bioRxiv_reformat |> 
  distinct(citing_journal) |>
  filter(
    citing_journal != "Proceedings of the National Academy of Sciences of the United States of America" |
    citing_journal != "Nature" |
    citing_journal != "Science" |
    citing_journal != "PLoS ONE" |
    citing_journal != "Nature Communications"
)
...
philo_to_bio <- philo_to_bio |> mutate(citing_journal = case_when(
    citing_journal == "STUDIES IN HISTORY AND PHILOSOPHY OF SCIENCE PART C :STUDIES IN HISTORY AND PHILOSOPHY OF BIOLOGICAL AND BIOMEDICAL SCIENCES" ~ 
      "STUDIES IN HPS : PART C",
     citing_journal == "HISTORY AND PHILOSOPHY OF THE LIFE SCIENCES" ~ "HPLS",
    TRUE ~ citing_journal
  ))
...
fig <- ggplot(philo_to_bio |> filter(citing_year> 1986 & citing_year < 2025) |> select(citing_year, ratio, citing_journal) |> distinct(), aes(x = citing_year, y = ratio)) + #color = citing_journal)) +
  ...
  geom_smooth() + 
  labs(
    #title = "Citations Towards Biology in Philosophy of Biology Over Time", # No title for master thesis. 
    x = "Citing Year",
...
![](C:/Users/jacob/OneDrive/Bureau/computational_projects/Estrangement/Visualisation/CHP3/BERTopic_biology_and_philosophy.png)
...
![](C:/Users/jacob/OneDrive/Bureau/computational_projects/Estrangement/Visualisation/CHP3/map_philo_bio_update.png)
...
p <- ggplot(meta_topics, aes(x = reorder(MetaTopic, Percent), y = Percent, color = MetaTopic, fill = MetaTopic)) + 
  geom_col() +
  coord_flip() + 
  labs(title = "Meta-Topic in Philosophy of Biology (%)", x = "MetaTopics", y = "Percent") + 
...
### Evolution of Meta-Topics
```{r}
meta_topic_evo <- read_csv("C:/Users/jacob/OneDrive/Bureau/computational_projects/Estrangement/Data/meta_topic_evolution.csv")


meta_topic_evo$clean_year <- year(meta_topic_evo$Year)
meta_topic_evo_filtered <- meta_topic_evo |> filter(Meta_Topic != "Unlabelled", 
                                                    clean_year != 2025)

meta_topic_evo_filtered <- meta_topic_evo_filtered |> group_by(clean_year) |>
 mutate(Count_all = sum(Count),
        ratio = Count/Count_all*100) |> ungroup()


```

```{r}
order_df <- meta_topic_evo_filtered %>%
  group_by(Meta_Topic) %>%
  summarise(max_ratio = max(ratio, na.rm = TRUE)) %>%
  arrange(desc(max_ratio))

meta_topic_evo_filtered$Meta_Topic <- factor(
  meta_topic_evo_filtered$Meta_Topic,
  levels = order_df$Meta_Topic
)

ggplot(meta_topic_evo_filtered, aes(x = clean_year, y = ratio, color = Meta_Topic)) +
  #geom_line() +
  geom_smooth(alpha = 0.3, se = FALSE) +
  labs(color = "Meta-Topics", x = "Year", y = "Ratio")
