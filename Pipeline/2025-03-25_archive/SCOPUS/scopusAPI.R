	...
	if (!content %in% c("complete", "standard")) {
	  ...
	  insttoken_key <- inst_token_header(insttoken)
		...
		while (resultCount > retrievedCount && retrievedCount < retMax) { ## check if it's necessary to perform multiple requests to retrieve all of the results; if so, create a loop to retrieve additional pages of results
			...
			theURL <- httr::GET("https://api.elsevier.com/content/search/scopus", query = list(apiKey = key,  insttoken = insttoken_key, query = string,  sort = mySort, httpAccept = "application/xml", view = content, count = retCount, cursor = cursor)) ## get the next page of results
